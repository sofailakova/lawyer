...
function bind_preview(selector, config, e) {
    ...
    $(selector + '-select-area img').load(function(){
        $(this).unbind('load');
        ...
        $(this).imgAreaSelect({
            aspectRatio: config.select_area_width + ":" + config.select_area_height,
            fadeSpeed: 100,
            x1: sel.x1,
            y1: sel.y1,
            x2: sel.x2,
...
function update_coors(selector, config, img, selection) {
    ...
    if (parseInt(selection.select_area_width) > 0) {
        var ratiox = config.select_area_width / (selection.select_area_width || 1);
        var ratioy = config.select_area_height / (selection.select_area_height || 1);

        $(selector + '-preview img').css({
            width: Math.round(ratiox * img.select_area_width) + 'px',
            height: Math.round(ratioy * img.select_area_height) + 'px',
